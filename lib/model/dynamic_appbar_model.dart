  ...
  bool onScrollNotification(
    ...
    if (notification is ScrollUpdateNotification) {
      ...
      if (scrollDelta.abs() > 10
              ...
              /// user has scroll more than 10 pixel
              &&
              (metrics.pixels > metrics.minScrollExtent &&
                  metrics.pixels < metrics.maxScrollExtent)

              /// AND (scrollView is not < min OR > max) (this prevent from 'bounceEffect' unwanted notifications)
              &&
              ((isVisible && scrollDelta > 0) ||
                  (!isVisible && scrollDelta < 0))

          /// AND user is scrolling in a way that need to trigers .invert()
          ) {
        toggleIsVisible();
      }
    }
